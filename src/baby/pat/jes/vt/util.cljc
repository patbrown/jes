...
(def coerce c/coerce)
(defn-spec coerce-with ::vt/any [m ::vt/map vt ::vt/qkw thing ::vt/any]
  (c/coerce vt thing {:idents m}))
(defn-spec close-map-as ::vt/map [vt ::vt/qkw m ::vt/map]
  (c/coerce vt {:closed true}))
...
(defn-spec filter-keys-by-ns ::vt/any
  ...
  (if (string? m)
    (medley.core/filter-keys #(= nmsp (namespace %)) m)
    (medley.core/filter-keys #(= nmsp (keyword (namespace %))) m)))
